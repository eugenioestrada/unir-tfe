...
public class ResponsiveDesignTests : PlaywrightTest
{
    ...
    [Test]
    [Description("Validates design on mobile portrait (375x667 - iPhone SE)")]
    public async Task MobilePortrait_ShouldLookAmazing()
    {
        ...
        Assert.That(heroPadding, Is.Not.Empty, "Hero should have appropriate padding on mobile");
        ...
        Assert.That(fontSizeValue, Is.GreaterThanOrEqualTo(24), 
        ...
        Assert.That(buttonHeight, Is.GreaterThanOrEqualTo(48), 
        ...
        if (cardCount > 1)
        {
            ...
            Assert.That(secondCardTop, Is.GreaterThanOrEqualTo(firstCardBottom - 5), 
                "Cards should stack vertically on mobile");
        }
    }

    [Test]
    [Description("Validates design on mobile landscape (667x375)")]
    public async Task MobileLandscape_ShouldOptimizeForSmallHeight()
    {
        ...
        Assert.That(containerPadding, Is.Not.Empty, "Container should have optimized padding for landscape");

        // Verify title is scaled appropriately
        var title = Page.Locator(".game-title");
        var fontSize = await title.EvaluateAsync<string>("el => window.getComputedStyle(el).fontSize");
        var fontSizeValue = double.Parse(fontSize.Replace("px", ""));
        Assert.That(fontSizeValue, Is.GreaterThan(0).And.LessThanOrEqualTo(48), 
            "Title should be scaled down for landscape orientation");
    }

    [Test]
    [Description("Validates design on tablet (768x1024 - iPad)")]
    public async Task Tablet_ShouldProvideOptimalExperience()
    {
        ...
        var heroPadding = await hero.EvaluateAsync<string>("el => window.getComputedStyle(el).padding");
        Assert.That(heroPadding, Is.Not.Empty, "Hero should have tablet-optimized padding");
        ...
        if (await grid.CountAsync() > 0)
        {
            ...
            Assert.That(gridTemplate, Does.Contain("px").Or.Contain("fr"), 
        ...
        Assert.That(minHeightValue, Is.GreaterThanOrEqualTo(52), 
            "Large buttons should be easy to tap on tablet");
    }

    [Test]
    [Description("Validates design on desktop (1920x1080 - Full HD)")]
    ...
    [Test]
    [Description("Validates design on TV/10-foot UI (1920x1080+)")]
    public async Task TV_ShouldProvide10FootUIExperience()
    {
        ...
        if (await roomCode.CountAsync() > 0)
        {
            ...
            Assert.That(codeFontSize, Is.GreaterThanOrEqualTo(48), 
                "Room code should be extra large for TV viewing (>=48px)");
        }
    }

    [Test]
    [Description("Validates that content doesn't overflow on small screens")]
    public async Task SmallScreens_ShouldNotOverflow()
    {
        ...
        Assert.That(containerWidth, Is.LessThanOrEqualTo(viewportWidth), 
            "Container should fit within viewport width");
    }

    [Test]
    [Description("Validates that images are responsive and don't break layout")]
    public async Task Images_ShouldBeResponsive()
    {
        ...
        foreach (var viewport in viewports)
        {
            ...
            if (await qrImage.CountAsync() > 0)
            {
                ...
                Assert.That(imageWidth, Is.LessThanOrEqualTo(containerWidth + 1), 
    ...
    [Test]
    [Description("Validates that touch targets meet minimum size requirements")]
    public async Task TouchTargets_ShouldMeetMinimumSize()
    {
        ...
        foreach (var button in buttons)
        {
            if (await button.IsVisibleAsync())
            {
                ...
                Assert.That(width, Is.GreaterThanOrEqualTo(44), 
    ...
    [Test]
    [Description("Validates that grid layout adapts correctly across breakpoints")]
